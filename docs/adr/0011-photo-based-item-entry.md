...
- 关联：ADR 0009（单品实物图不进生成链路 / 拼贴为主 + 旧试穿试点）、ADR 0010（温度驱动 / 依赖单品 season 标签准确）

## 背景（为什么不再用手动表单）

1. **手动录入太烦**：单品有 category / season / material / color / fit / scene / tags 等多个字段，
   一件件手动区分（尤其"这件是春秋还是四季""加绒还是薄"）是重度负担，使用者明确拒绝。
2. **54 件单品 `img` 全为 null**（云端真源 `seedImport/items.json`），拼贴/旧试穿/识别都缺素材。
...
| 旧试穿素材（若平拍） | 同一张 `img` 满足早期专用试穿模型平拍要求（ADR 0009 试点） |
...
所有单品按旧试穿标准**平拍**：单件铺开、背景干净、四周少留白（满足 早期专用试穿模型 平拍要求）。
一张照片同时是实物图 + 拼贴素材 + 旧试穿素材 + 识别源，四处受益闭环。

平拍硬指标（来自 早期专用试穿模型 官方 + ADR 0009）：
- 服饰平铺、单一主体且完整
- 背景干净，四周不宜留白过多
